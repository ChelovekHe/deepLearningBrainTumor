...
from data_generators import load_all_patients_BraTS_2014_HGG, SegmentationBatchGeneratorBraTS2014, load_all_patients_BraTS_2014_HGG_non_adapted_values
from data_augmentation_generators import seg_channel_selection_generator, center_crop_generator, rotation_generator, elastric_transform_generator, center_crop_seg_generator, pad_generator
...
all_patients = load_all_patients_BraTS_2014_HGG_non_adapted_values()
...
class_frequencies = np.array(class_frequencies).astype(np.float32)**0.5
...
while epoch < 50:
    ...
    if (epoch > 1) and (all_validation_losses[-1] / all_validation_losses[-2] > 1.5):
        ...
        compare_seg_with_gt(max_n_images=5, epoch=epoch)
        epoch += 1
        learning_rate *= 0.7
