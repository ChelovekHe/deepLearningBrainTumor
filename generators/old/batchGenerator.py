    ...
    def __init__(self, batchsize, database, testing=False, process_func=None, use_caffe=True):
        ...
        with self.in_db_data.begin() as in_txn:
            ...
            if self.use_caffe:
                ...
                img_dat = bytes(value)
                (img, seg, label) = pickle.loads(img_dat)
                # img = img[np.newaxis, :, :]
        ...
        self.segmentation_batches = np.zeros((self.batchsize, self.img_width, self.img_height), dtype=theano.config.floatX)

        self.labels_batches = np.zeros((self.batchsize,), dtype=np.int32)
        self.keys_batches = ['']*self.batchsize
    ...
    def next(self):
        ...
        try:
            for i in xrange(self.batchsize):
                ...
                self.segmentation_batches[i, :, :] = seg
                self.labels_batches[i] = np.int32(label)
                # self.keys_batches[i] = key
        ...
        return [self.data_batches, self.labels_batches, self.segmentation_batches]
