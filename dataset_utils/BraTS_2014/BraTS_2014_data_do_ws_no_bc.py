...
for folder in folders:
    ...
    output_files = []
    for id in range(300):
        if not path.isfile(path.join(folder, "%03.0d_T1.nii.gz"%id)):
            continue
        if not path.isfile(path.join(folder, "%03.0d_T1c.nii.gz"%id)):
            continue
        if not path.isfile(path.join(folder, "%03.0d_T2.nii.gz"%id)):
            continue
        if not path.isfile(path.join(folder, "%03.0d_Flair.nii.gz"%id)):
            continue
        other_files_other.append(path.join(folder, "%03.0d_T1c.nii.gz"%id))
        ...
        output_files.append(path.join(folder, "%03.0d_T1c_ws"%id))
        output_files.append(path.join(folder, "%03.0d_T2_ws"%id))
        output_files.append(path.join(folder, "%03.0d_Flair_ws"%id))

        t1_files.append(path.join(folder, "%03.0d_T1.nii.gz"%id))
        t1_files_output.append(path.join(folder, "%03.0d_T1_ws"%id))

        other_files_t1.append(path.join(folder, "%03.0d_T1.nii.gz"%id))
        other_files_t1.append(path.join(folder, "%03.0d_T1.nii.gz"%id))
        other_files_t1.append(path.join(folder, "%03.0d_T1.nii.gz"%id))
