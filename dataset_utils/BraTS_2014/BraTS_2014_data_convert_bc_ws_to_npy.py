...
def convert_patient_to_npy(folder, out_folder, id):
    ...
    if not path.isfile(path.join(folder, "%03.0d_T2_bc_ws.nii.gz"%id)):
        return
    if not path.isfile(path.join(folder, "%03.0d_Flair_bc_ws.nii.gz"%id)):
        return
    ...
    flair_img = sitk.GetArrayFromImage(sitk.ReadImage(path.join(folder, "%03.0d_Flair_bc_ws.nii.gz"%id))).astype(np.float32)
    ...
    np.save(path.join(out_folder, "%03.0d_Flair_bc_ws.npy"%id), flair_img)
