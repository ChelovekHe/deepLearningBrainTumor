...
for folder in folders:
    ...
    for id in range(300):
        ...
        if not path.isfile(path.join(folder, "%03.0d_T2_bc.nii.gz"%id)):
            continue
        if not path.isfile(path.join(folder, "%03.0d_Flair_bc.nii.gz"%id)):
            continue
        ...
        other_files_other.append(path.join(folder, "%03.0d_Flair_bc.nii.gz"%id))
        output_files.append(path.join(folder, "%03.0d_T1c_bc_ws"%id))
        output_files.append(path.join(folder, "%03.0d_T2_bc_ws"%id))
        output_files.append(path.join(folder, "%03.0d_Flair_bc_ws"%id))
