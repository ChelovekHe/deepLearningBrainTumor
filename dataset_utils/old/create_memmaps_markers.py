...
train_pos_shape = (expected_n_samples, 18, PATCH_SIZE, PATCH_SIZE)
val_neg_shape = (expected_n_samples * 15/100., 18, PATCH_SIZE, PATCH_SIZE)
val_pos_shape = (expected_n_samples * 15/100., 18, PATCH_SIZE, PATCH_SIZE)

memmap_name = "patchClassification_ws_resampled_t1km_flair_adc_cbv_markers"

train_neg_memmap_RTK2 = memmap("%s_train_neg.memmap" % (memmap_name + "_RTK2"), dtype=np.float32, mode="w+", shape=train_neg_shape)
train_pos_memmap_RTK2 = memmap("%s_train_pos.memmap" % (memmap_name + "_RTK2"), dtype=np.float32, mode="w+", shape=train_pos_shape)
val_neg_memmap_RTK2 = memmap("%s_val_neg.memmap" % (memmap_name + "_RTK2"), dtype=np.float32, mode="w+", shape=val_neg_shape)
val_pos_memmap_RTK2 = memmap("%s_val_pos.memmap" % (memmap_name + "_RTK2"), dtype=np.float32, mode="w+", shape=val_pos_shape)
train_neg_memmap_MGMT = memmap("%s_train_neg.memmap" % (memmap_name + "_MGMT"), dtype=np.float32, mode="w+", shape=train_neg_shape)
train_pos_memmap_MGMT = memmap("%s_train_pos.memmap" % (memmap_name + "_MGMT"), dtype=np.float32, mode="w+", shape=train_pos_shape)
val_neg_memmap_MGMT = memmap("%s_val_neg.memmap" % (memmap_name + "_MGMT"), dtype=np.float32, mode="w+", shape=val_neg_shape)
val_pos_memmap_MGMT = memmap("%s_val_pos.memmap" % (memmap_name + "_MGMT"), dtype=np.float32, mode="w+", shape=val_pos_shape)
train_neg_memmap_EGFR = memmap("%s_train_neg.memmap" % (memmap_name + "_EGFR"), dtype=np.float32, mode="w+", shape=train_neg_shape)
train_pos_memmap_EGFR = memmap("%s_train_pos.memmap" % (memmap_name + "_EGFR"), dtype=np.float32, mode="w+", shape=train_pos_shape)
val_neg_memmap_EGFR = memmap("%s_val_neg.memmap" % (memmap_name + "_EGFR"), dtype=np.float32, mode="w+", shape=val_neg_shape)
val_pos_memmap_EGFR = memmap("%s_val_pos.memmap" % (memmap_name + "_EGFR"), dtype=np.float32, mode="w+", shape=val_pos_shape)

n_negative_train_RTK2 = 0
n_negative_val_RTK2 = 0
n_positive_train_RTK2 = 0
n_positive_val_RTK2 = 0
n_negative_train_MGMT = 0
    ...
    for i in xrange(30):
        ...
        if not test:
            if patient_markers[idx_in_patient_markers][1] == 0:
                train_neg_memmap_RTK2[n_negative_train_RTK2:n_negative_train_RTK2+30] = result
                n_negative_train_RTK2 += 30
            elif patient_markers[idx_in_patient_markers][1] == 1:
                train_pos_memmap_RTK2[n_positive_train_RTK2:n_positive_train_RTK2+30] = result
                n_positive_train_RTK2 += 30
            if patient_markers[idx_in_patient_markers][2] == 0:
                train_neg_memmap_MGMT[n_negative_train_MGMT:n_negative_train_MGMT+30] = result
                n_negative_train_MGMT += 30
            elif patient_markers[idx_in_patient_markers][2] == 1:
                train_pos_memmap_MGMT[n_positive_train_MGMT:n_positive_train_MGMT+30] = result
                n_positive_train_MGMT += 30
            if patient_markers[idx_in_patient_markers][3] == 0:
                train_neg_memmap_EGFR[n_negative_train_EGFR:n_negative_train_EGFR+30] = result
                n_negative_train_EGFR += 30
            elif patient_markers[idx_in_patient_markers][3] == 1:
                train_pos_memmap_EGFR[n_positive_train_EGFR:n_positive_train_EGFR+30] = result
                n_positive_train_EGFR += 30
        else:
            if patient_markers[idx_in_patient_markers][1] == 0:
                val_neg_memmap_RTK2[n_negative_val_RTK2:n_negative_val_RTK2+30] = result
                n_negative_val_RTK2 += 30
            elif patient_markers[idx_in_patient_markers][1] == 1:
                val_pos_memmap_RTK2[n_positive_val_RTK2:n_positive_val_RTK2+30] = result
                n_positive_val_RTK2 += 30
            if patient_markers[idx_in_patient_markers][2] == 0:
                val_neg_memmap_MGMT[n_negative_val_MGMT:n_negative_val_MGMT+30] = result
                n_negative_val_MGMT += 30
            elif patient_markers[idx_in_patient_markers][2] == 1:
                val_pos_memmap_MGMT[n_positive_val_MGMT:n_positive_val_MGMT+30] = result
                n_positive_val_MGMT += 30
            if patient_markers[idx_in_patient_markers][3] == 0:
                val_neg_memmap_EGFR[n_negative_val_EGFR:n_negative_val_EGFR+30] = result
                n_negative_val_EGFR += 30
            elif patient_markers[idx_in_patient_markers][3] == 1:
                val_pos_memmap_EGFR[n_positive_val_EGFR:n_positive_val_EGFR+30] = result
                n_positive_val_EGFR += 30

    print "train RTK2 (total, pos, neg): ", n_negative_train_RTK2 + n_positive_train_RTK2, n_positive_train_RTK2, n_negative_train_RTK2
    print "test RTK2 (total, pos, neg): ", n_positive_val_RTK2 + n_negative_val_RTK2, n_positive_val_RTK2, n_negative_val_RTK2
    print "train MGMT (total, pos, neg): ", n_negative_train_MGMT + n_positive_train_MGMT, n_positive_train_MGMT, n_negative_train_MGMT
    print "test MGMT (total, pos, neg): ", n_positive_val_MGMT + n_negative_val_MGMT, n_positive_val_MGMT, n_negative_val_MGMT
    print "train EGFR (total, pos, neg): ", n_negative_train_EGFR + n_positive_train_EGFR, n_positive_train_EGFR, n_negative_train_EGFR
    print "test EGFR (total, pos, neg): ", n_positive_val_EGFR + n_negative_val_EGFR, n_positive_val_EGFR, n_negative_val_EGFR


my_dict_RTK2 = {
    "train_total" : n_negative_train_RTK2 + n_positive_train_RTK2,
    "train_pos": n_positive_train_RTK2,
    "train_neg": n_negative_train_RTK2,
    "val_total" : n_positive_val_RTK2 + n_negative_val_RTK2,
    "val_pos": n_positive_val_RTK2,
    "val_neg": n_negative_val_RTK2,
...
with open("%s_properties.pkl" % (memmap_name + "_RTK2"), 'w') as f:
    cPickle.dump(my_dict_RTK2, f)

my_dict_MGMT = {
    "train_total" : n_negative_train_MGMT + n_positive_train_MGMT,
    "train_pos": n_positive_train_MGMT,
    "train_neg": n_negative_train_MGMT,
    "val_total" : n_positive_val_MGMT + n_negative_val_MGMT,
    "val_pos": n_positive_val_MGMT,
    "val_neg": n_negative_val_MGMT,
    "train_neg_shape": train_neg_shape,
    "train_pos_shape": train_pos_shape,
    "val_neg_shape": val_neg_shape,
    "val_pos_shape": val_pos_shape
...
with open("%s_properties.pkl" % (memmap_name + "_MGMT"), 'w') as f:
    cPickle.dump(my_dict_MGMT, f)

my_dict_EGFR = {
    "train_total" : n_negative_train_EGFR + n_positive_train_EGFR,
    "train_pos": n_positive_train_EGFR,
    "train_neg": n_negative_train_EGFR,
    "val_total" : n_positive_val_EGFR + n_negative_val_EGFR,
    "val_pos": n_positive_val_EGFR,
    "val_neg": n_negative_val_EGFR,
    "train_neg_shape": train_neg_shape,
    "train_pos_shape": train_pos_shape,
    "val_neg_shape": val_neg_shape,
    "val_pos_shape": val_pos_shape
...
with open("%s_properties.pkl" % (memmap_name + "_EGFR"), 'w') as f:
    cPickle.dump(my_dict_EGFR, f)
