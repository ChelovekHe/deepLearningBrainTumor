# DESCR: Now using two separate memmaps (one with positive examples, one with negative). See if that is bettern than scaling the loss
# this works pretty well! training is slower now, but the network trains much better AND more stable
...
'''with open("classifyPatches_memmap_v0.4.py_Params.pkl", 'r') as f:
    params = cPickle.load(f)
lasagne.layers.set_all_param_values(net['prob'], params)
with open("classifyPatches_memmap_v0.4.py_allLossesNAccur.pkl", 'r') as f:
